...
import { catogoriesData, quickLinksData } from '@/constant'
            ...
            <div>
              ...
              <div className='flex flex-col gap-3'>
                {catogoriesData.map((item)=> (
                  <Link className='text-gray-600 hover:text-[#151515] text-sm font-medium hoverEffect' href={`/category/${item.href}`} key={item.title}>
                    {item.title}
                  </Link>
                ))}
              </div>
