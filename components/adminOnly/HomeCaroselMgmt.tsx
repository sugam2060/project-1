...
import {
  uploadAndConvertHomeCaroselImages,
  getCaroselImages,
} from "@/actions/productActions/ManageHomeCarosel";
import { z } from "zod";
    ...
    <div className="">
      <Form {...form}>
        <form onSubmit={onSubmit}>
          ...
          {formError && (
            <div className="mx-3 py-1 mt-2 rounded-md bg-red-500/90 text-black font-semibold text-center px-2 space-y-1">
              {formError}
            </div>
          )}
          {formSuccess && (
            <div className="mx-3 py-1 mt-2 rounded-md bg-green-500/90 text-center font-semibold  px-2">
              {formSuccess}
            </div>
          )}
          <CardFooter>
            ...
            >
              Upload
              {isPending && (
                <Loader2 className="absolute right-8 top-1/2 -translate-x-1/2 -translate-y-1/2 animate-spin" />
              )}
            </Button>
          </CardFooter>
        </form>
        ...
        {images.map((image, idx) => (
          <div key={idx} className="flex items-center justify-between p-2">
            ...
            <Button
              ...
              onClick={() => removeHomeCaroselImage(image)}
            >
              <p>Remove</p>
              {removingImage === image && (
