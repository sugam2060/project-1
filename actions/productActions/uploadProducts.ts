...
export const uploadProductsRemote = async (
  products: z.infer<typeof ProductFieldsSchema>
) => {
  ...
  if (slugExists)
    return {
      error: "Slug already exists try different description and product name",
    };
  ...
  try {
      ...
      data: {
        ...
        images: {
          createMany: {
            data: imageUrls.map((url,idx) => ({ imageUrl: url,position:idx })),
          },
        },
        slug: validated.data.slug,
...
export const uploadProductsLocal = async (
  products: z.infer<typeof ProductFieldsSchema>
) => {
  const validated = ProductFieldsSchema.safeParse(products);
  if (!validated.success) return { error: "Invalid product data" };
  ...
  try {
    for (const image of products.image as File[]) {
      if (!image || image.size === 0)
        return { error: "Failed to upload images. Try again later" };
